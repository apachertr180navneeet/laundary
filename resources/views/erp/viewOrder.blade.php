                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <div class="Client_table_action_area">
                                                    <a href="{{ url('/erp/show-order/' . $order->id) }}" class="btn Client_table_action_icon px-2">
    ...
    document.addEventListener("DOMContentLoaded", function() {
        $(document).ready(function() {
            ...
            $('#orderSearch').on('keyup', function() {
                var searchText = $(this).val().toLowerCase();
                $.ajax({
                    url: '/erp/view-order', // The route for searching orders
                    type: 'GET',
                    data: {
                        search: searchText
                    },
                ...
                <tr>
                    ...
                    <td>
                        <div class="Client_table_action_area">
                            <a href="/erp/show-order/${order.id}" class="btn Client_table_action_icon px-2">
                                <i class="tf-icons ti ti-eye"></i>
                            </a>
                            ${order.payment_status !== 'Paid' || order.status !== 'delivered' ? `
                                    <button class="btn Client_table_action_icon px-2" onclick="window.location='/erp/edit-order/${order.id}'">
                                        <i class="tf-icons ti ti-pencil"></i>
                                    </button>
                                    <button class="btn Client_table_action_icon px-2 delete_order_btn" data-id="${order.id}" data-bs-toggle="modal" data-bs-target="#delete_order">
                                        <i class="tf-icons ti ti-trash"></i>
...
;
                    ...
                    $('#printReceipt').attr('href', '/erp/receipt/' + id);
                    $('#printTags').attr('href', '/erp/tagslist/' + id);
                    $('#yes').modal('show');
                });
            ...
            $('#settleAndDeliverButton').on('click', function() {
                    ...
                    $.ajax({
                        url: '/erp/settle-and-deliver-order/' + orderId,
                        method: 'POST',
                        data: {
