    ...
    <div class="content-wrapper page_content_section_hp">
            ...
            <div class="client_list_area_hp Add_order_page_section">
                <div class="card">
                    <div class="card-body">
                        ...
                        <form action="{{ route('order.update', $order->id) }}" method="POST" id="addOrderFormValidation"
                                ...
                                <div class="col-lg-6 col-md-6 mb-2">
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-xl-6 col-lg-6 col-md-6 col-12 mb-3">
                                            <div class="form-group">
                                                ...
                                                <input type="text" value="{{ old('mobile', $order->mobile ?? '') }}"
                                                    ...
                                                    oninput="validateClientNum(this)" readonly />
                                            </div>
                                        </div>
                                        <!-- Client Name -->
                                        <div class="col-xl-6 col-lg-6 col-md-6 col-12 mb-3">
                                            <div class="form-group">
                                                ...
                                                <input type="text" id="client_name"
                                                    ...
                                                    oninput="validateClientName(this)" readonly />
                                            </div>
                                        </div>
                                        <!-- Booking Date -->
                                        <div class="col-xl-6 col-lg-6 col-md-6 col-12 mb-3">
                                        ...
                                        @foreach ($orderItems as $orderItemKey => $orderItemValue)
                                            ...
                                            @php
                                                // echo "<pre>";
                                                // print_r($orderItemValue);
                                            @endphp
                                            <div class="col-xl-12">
                                                <div class="row border" data-item-name="{{ $orderItemValue->items_name }}" data-item-category="{{ $orderItemValue->category_name }}">
                                                    <div class="col-md-1">
                                                        <i class="fa fa-trash remove-item" aria-hidden="true"></i>
                                                    </div>
                                                    <div class="col-md-8">
                                                        <input type="hidden" name="itemname[]" value="{{ $orderItemValue->items_name }}" />
                                                        <input type="hidden" name="itemcategory[]" value="{{ $orderItemValue->category_name }}" />
                                                        <input type="hidden" name="itemservice[]" value="{{ $orderItemValue->operation_id }}" />
                                                        <input type="hidden" name="itemqty[]" value="{{ $orderItemValue->quantity }}" />
                                                        <input type="hidden" name="clothlundaryqty[]" value="{{ $orderItemValue->weight }}" />
                                                        <input type="hidden" name="itemprice[]" value="{{ $orderItemValue->operation_price }}" />
                                                        <input type="hidden" name="qtyxprice[]" value="{{ $orderItemValue->price }}" />
                                                        <input type="hidden" name="unit[]" value="{{ $orderItemValue->unit }}" />
                                                        <p>
                                                            <span id="itemname">{{ $orderItemValue->items_name }}</span>
                                                            @if ($orderItemValue->unit != "Kg")
                                                                <span id="itemcategory">({{ $orderItemValue->category_name }})</span>
                                                            @endif
                                                        </p>
                                                        ...
                                                        @if ($orderItemValue->unit == "Kg")
                                                            @php
                                                                $lundaryItems = DB::table('lundary_order_item')
                                                                                ->join('categories', 'categories.id', '=', 'lundary_order_item.ProductCategroyId')
                                                                                ->join('items', 'items.id', '=', 'lundary_order_item.ProductName')
                                                                                ->select('lundary_order_item.*', 'categories.name as category_name', 'items.name as items_name')
                                                                                ->where('lundary_order_item.order_item_id', $order->id)
                                                                                ->get();
                                                            @endphp
                                                            @foreach ( $lundaryItems as $lundaryItemKey => $lundaryItemsValue)
                                                                <div class="product-detail">
                                                                    <p>Product: {{$lundaryItemsValue->items_name}}, Qty: {{$lundaryItemsValue->ProductQty}}, Category: {{$lundaryItemsValue->category_name}}</p>
                                                                    <input type="hidden" value="{{$lundaryItemsValue->items_name}}" name="lundaryProductName[]" />
                                                                    <input type="hidden" value="{{$lundaryItemsValue->ProductQty}}" name="lundaryProductQty[]" />
                                                                    <input type="hidden" value="{{$lundaryItemsValue->category_name}}" name="lundaryProductCategroyId[]" />
                                                                </div>
                                                            @endforeach
                                                        @endif


                                                        <p>Service: (<span id="itemservice">{{ $orderItemValue->operation_id }}</span>)</p>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <p><span id="itemqty[]">{{ $orderItemValue->weight }}</span> x <span id="itemprice[]">{{ $orderItemValue->operation_price }}</span> = <span id="qtyxprice[]">{{ $orderItemValue->price }}</span></p>
                                                    </div>
                                                </div>
                                            </div>
                                        @endforeach
                                        ...
                                        <div class="col-xl-12 col-lg-12 col-md-12 col-12 mb-3">
                                            <div class="row justify-content-between">
                                                <input type="hidden" name="gross_total" id="gross_total" value="{{$order->total_price}}" />
                                                <div class="col-xl-4 col-lg-4 col-md-4 col-12">
                                                    <h6>Gross Total:</h6>
                                                </div>
                                                <div class="col-xl-4 col-lg-4 col-md-4 col-12 text-end">
                                                    <h6 id="grossTotal">{{$order->total_price}}</h6>
                                                </div>
                                            </div>
                                            <div class="row justify-content-between">
                                                <div class="col-xl-4 col-lg-4 col-md-4 col-12">
                                                ...
                                                <div id="discountAmount" class="col-xl-4 col-lg-4 col-md-4 col-12 text-end">
                                                    <h6>{{$orderDiscountamount}}</h6>
                                                </div>
                                            </div>
                                            <div class="row justify-content-between">
                                                <input type="hidden" value="{{$order->total_qty}}" name="total_qty" id="total_qty" />
                                                <div class="col-xl-4 col-lg-4 col-md-4 col-12">
                                                    <h6>Total Count:</h6>
                                                </div>
                                            ...
                                            <div class="form-group">
                                                ...
                                                <select name="discount" id="discount" class="form-select">
                                                    ...
                                                    @foreach ($discounts as $discount)
                                                        <option value="{{ $discount->amount }}" {{ old('discount', $order->discount_id ?? '') == $discount->id ? 'selected' : '' }}>{{ $discount->name }}
                                                        </option>
                                                    @endforeach
                                                </select>
                                            </div>
        ...
        <script>
            ...
            document.getElementById('addRightOdrbtn').addEventListener('click', function() {
                ...
                // Validate inputs
                if (!itemName || !itemCategory || !itemService || isNaN(itemPrice) || itemPrice <= 0 || isNaN(
                    itemQty) || itemQty <= 0) {
            ...
            document.getElementById('addGarmentBtn').addEventListener('click', function() {
                ...
                var filteredItems = groupedProductItems.filter(function(item) {
                    return !item.grouped_details.hasOwnProperty('Laundry By Weight') &&
                        ...
                        item.product_item.name !== 'Socks/Stocking Pair' && item.product_item.name !==
                        'Slip on';
                });
            ...
            increaseBtn.addEventListener("click", function() {
                let productName = document.getElementById('popupProductName').textContent
            .trim(); // Get the product name
                if (productName === 'Laundry By Weight') {
                    // Add d-none class to hide the buttons
                    increaseBtn.classList.add('d-none');
                    decreaseBtn.classList.add('d-none');
            ...
            decreaseBtn.addEventListener("click", function() {
                if (productName === 'Laundry By Weight') {
                    ...
                    if (!isNaN(currentValue) && currentValue > 0) {
                        qtyInput.value = Math.max(currentValue - 1,
                        0); // Decrement by 1 for other items, but not below 0
                    }
                }
            });
            ...
            document.getElementById('addLaundaryItem').addEventListener('click', function() {
                ...
                qtyInputs.forEach(function(input) {
                    ...
                    if (qty > 0) {
                        var productName = input.closest('.col-md-8').querySelector('h6.landsearchProductName')
