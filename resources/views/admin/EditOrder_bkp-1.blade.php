        ...
        <div class="client_list_area_hp Add_order_page_section">
            <div class="card">
                <div class="card-body">
                    ...
                    @if (isset($order))
                    <form action="{{ route('order.update', $order->id) }}" method="POST" enctype="multipart/form-data" id="orderForm">
                        @method('PUT')
                        @else
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <div class="Add_order_btn_area text-end">
                                                <button class="btn w-100" type="button" data-bs-toggle="modal" data-bs-target="#CreateOrder">Save</button>
                                            </div>
                                        </div>
                                        <!-- Create Order Model -->
                                            ...
                                            <div class="row">
                                                <div class="col-lg-9 col-md-9 mainopdiv">
                                                    ...
                                                    <div class="categorysection">
                                                        ...
                                                        <span onclick="categoryItem('{{ $categoryName }}','{{ $productItem->id }}', this)" class="badge text-dark mb-2 subcategory bg-light">{{ $categoryName }}</span>
                                                        @endforeach
                                                    </div>
                                                ...
                                                <div class="col-lg-3 col-md-3 text-center">
                                                    ...
                                                    <div class="Add_order_btn_area">
                                                        <button type="button" id="addbtnpreview" class="btn add-product-btn" data-bs-toggle="offcanvas" data-bs-target="#offcanvasRight" aria-controls="offcanvasRight" data-product-name="{{ $productItem->name }}" data-images="{{ url('images/categories_img/'.$productItem->image) }}">Add</button>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
            ...
            $("#addOrderFormValidation").validate({
                ...
                submitHandler: function(form) {
                    ...
                    Object.keys(items).forEach(key => {
                        ...
                        if (item.qty > 0) {
                            $('#orderForm, #addOrderFormValidation').append(`<input type="hidden" name="items[${item.productName}][${item.category}][${item.service}][qty]" value="${item.qty}">`);
                            $('#orderForm, #addOrderFormValidation').append(`<input type="hidden" name="items[${item.productName}][${item.category}][${item.service}][unit_price]" value="${item.unitPrice}">`);
                        }
                    });
                    form.submit();
                }
